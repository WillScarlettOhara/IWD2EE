...
function IEex_InjectOptionIniComments()
	...
	local comments = {
		...
		["IP Bump Moving Allies"]                 = "Improved Pathfinding: let party members shove each other aside WHILE WALKING. Vanilla makes a moving sprite non-bumpable, so during a group move all six bodies become hard walls at once: the shove is refused before it starts and the party welds itself into a clump that walks on the spot. Vanilla only survives it because its collision ladder throws the path away every couple of seconds, which drops the character to idle -- the stutter IS the deadlock breaker. Attacking characters stay unshovable (that is Combat Slide's business), and enemies never shove the party. 1 = on (default).",
		...
		["Destination Marker Thickness"]          = "Stroke width of the move-destination / target reticle, in MAP pixels (zoom-scaled, like the selection circles). 0 = one step lighter than Selection Circle Thickness, never below 1 (default -- the BG2EE look, where the destination chevrons echo the circle rather than match it; with the default 1 px circles that floor keeps it at the vanilla 1 px), or force 1 to 4. OpenGL only.",
