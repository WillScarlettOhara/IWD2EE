...
function IEex_InjectOptionIniComments()
	...
	local comments = {
		...
		["Selection Circle Thickness"]            = "Stroke width of the selection circles under characters, in HALF map pixels: it magnifies with the camera zoom, like everything else on the map, so the outline keeps the same weight relative to the circle at every zoom level (3 here is about 5 screen pixels at 3x). The count is a whole number of 1-pixel rings, so a thin setting stays pinned at 1 pixel over a range of zooms before stepping up. 1 = the vanilla hairline (default); 0 = automatic (2 px, or 3 px above 1920 screen width -- the BG2EE weighting); or force up to 4. Independent of Colored Selection Circles: either can be had without the other. Thickness grows INWARD, so the outer edge -- and the click target -- never moves. OpenGL only.",
		["Destination Marker Thickness"]          = "Stroke width of the move-destination / target reticle, in HALF map pixels, on the same scale as Selection Circle Thickness. 0 = one step lighter than Selection Circle Thickness, never below 1 (default -- the BG2EE look, where the destination chevrons echo the circle rather than match it; with the default 1 px circles that floor keeps it at the vanilla 1 px), or force 1 to 4. Above one pixel each of the four pie pieces is drawn as one closed path with its three corners joined and rounded (the engine draws them as three separate primitives that only meet because they are one pixel wide, so a wider stroke pulls them apart), and the width is capped at a quarter of a pie piece's depth, past which the pieces fill into solid wedges. Exactly one pixel is the engine's own geometry, unchanged. OpenGL only.",
