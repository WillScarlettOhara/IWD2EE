...
function IEex_InjectOptionIniComments()
	...
	local comments = {
		...
		["Cutscene Zoom"]                         = "Hold a zoom level for the duration of a cutscene, so scripted shots are framed the way they were composed (they were authored for the 800x600 view: at high resolution the same shot plays inside a window several times too wide and the beats land wrong). 0 = off, keep whatever zoom the player is using (default); 1 = the authored frame, matching its HEIGHT so nothing is ever cropped -- only the sides widen (3.6x at 2160p, 2.4x at 1440p, 1.8x at 1080p); 2 = the same snapped to a whole factor, so the map stays pixel-exact (3x at 2160p, 2x at 1440p; below 1200p that is 1x = no zoom). The player's own zoom is restored when the cutscene ends, and the wheel is ignored while the lock holds. OpenGL only.",
		...
		["Max FPS"]                               = "Frame cap: 0 = auto (just under display refresh), 9999 = uncapped, or an explicit ceiling (e.g. 144). (No longer in the options menu -- ini only.)",
