...
function IEex_InjectOptionIniComments()
	...
	local comments = {
		...
		["IP Party Bump"]                         = "Improved Pathfinding: let party members shove each OTHER aside while walking, so narrow doorways stop being single-file queues. Vanilla makes a WALKING sprite non-bumpable: it becomes a hard wall on the search map, and a follower that reaches it is stopped dead and has to re-search. A shoved ally is displaced one cell and KEEPS its path -- it never stops, and it is never pulled back. Only allies can shove a PC, never an enemy. 1 = on (default). Restart required.",
