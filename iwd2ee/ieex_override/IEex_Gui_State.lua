...
function IEex_InjectOptionIniComments()
	...
	local comments = {
		...
		["Destination Marker Thickness"]          = "Stroke width of the move-destination / target reticle, in MAP pixels (zoom-scaled, like the selection circles). 0 = one step lighter than Selection Circle Thickness, never below 1 (default -- the BG2EE look, where the destination chevrons echo the circle rather than match it; with the default 1 px circles that floor keeps it at the vanilla 1 px), or force 1 to 4. The reticle is not a circle but a stroke glyph -- four hollow pie pieces only a few map pixels deep -- so it is thickened differently: the stroke is centred on the vanilla outline (not grown inward), the four central tips get a round join at the same width as the rest of the outline, and the width is capped at a quarter of a pie piece's depth, past which the lobes fill into solid wedges. That cap scales with the glyph, so at low zoom a high value simply stops having any further effect. OpenGL only.",
