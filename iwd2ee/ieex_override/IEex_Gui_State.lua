...
if not IEex_Vanilla then
	...
	local ex_ini_option_defaults = {
		...
		{"Selection Circle Thickness", 2},
		{"Destination Marker Thickness", 2},
...
function IEex_InjectOptionIniComments()
	...
	local comments = {
		...
		["Selection Circle Thickness"]            = "Stroke width of the selection circles under characters, in HALF map pixels: it magnifies with the camera zoom, like everything else on the map, so the outline keeps the same weight relative to the circle at every zoom level. The count is a whole number of 1-pixel rings, so a thin setting stays pinned at 1 pixel over a range of zooms before stepping up. 2 = one whole map pixel, i.e. the vanilla hairline at zoom 1, and the default; 1 is thinner still; 0 = automatic by pixel density (the BG2EE weighting); or force up to 4. Independent of Colored Selection Circles: either can be had without the other. Thickness grows INWARD, so the outer edge -- and the click target -- never moves. OpenGL only.",
		["Destination Marker Thickness"]          = "Stroke width of the move-destination / target reticle, in HALF map pixels, on the same scale and with the same default as Selection Circle Thickness. 0 = follow Selection Circle Thickness exactly; or force 1 to 4. Each of the four pie pieces is drawn as one closed path with its three corners joined and rounded -- the engine draws them as three separate primitives that only meet because they are one pixel wide, so a wider stroke pulls them apart -- and the width is capped at a quarter of a pie piece's depth, past which the pieces fill into solid wedges. OpenGL only.",
